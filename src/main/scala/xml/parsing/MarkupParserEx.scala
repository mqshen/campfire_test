 ...
 * @version 1.0
 */
trait MarkupParserEx extends MarkupParserCommon with TokenTests
{ 
  self: MarkupParserEx with MarkupHandler =>
  ...
  */
  ...
  def element1(pscope: NamespaceBinding): NodeSeq = {
    ...
    val res = handle.elem(pos, pre, local, aMap, scope, false, ts)
